...
func asInt64(v uint64) int64 {
	...
	byt := []byte{
		byte(v >> 56), byte(v >> 48), byte(v >> 40), byte(v >> 32), //nolint:gosec // safe: v is masked to lower 63 bits
		byte(v >> 24), byte(v >> 16), byte(v >> 8), byte(v), //nolint:gosec // safe: v is masked to lower 63 bits
	}
	var out int64
	for i, b := range byt {
		out |= int64(b) << (56 - i*8)
